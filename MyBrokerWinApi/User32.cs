...
namespace MyBroker.Win32
{
    ...
    public static class User32
    {
        ...
        public static IntPtr FindFirstVisibleWindowWithClassName(IntPtr parentWnd, string clsName)
        {
            IntPtr result = new IntPtr();
            StringBuilder className = new StringBuilder(256);

            IntPtr ptr = User32.GetWindow(parentWnd, GetWindow_Cmd.GW_CHILD);
            while (ptr.ToInt32() > 0)
            {
                GetClassName(ptr, className, 256);
                WindowStyles ws = (WindowStyles)GetWindowLong(ptr, GWL_STYLE);
                if ((((uint)ws & (uint)WindowStyles.WS_VISIBLE) == (uint)WindowStyles.WS_VISIBLE)
                    &&
                    className.ToString() == clsName
                    )
                {
                    result = ptr;
                    break;
                }
                ptr = User32.GetWindow(ptr, GetWindow_Cmd.GW_HWNDNEXT);
            }
        ...
        {
            WindowStyles ws = (WindowStyles)GetWindowLong(hWnd, GWL_STYLE);
            return (((uint)ws & (uint)WindowStyles.WS_VISIBLE) == (uint)WindowStyles.WS_VISIBLE);
        }

        public static int SendMouseLeftClick(IntPtr ptr, int x, int y)
        {
            uint lParam = MakeLParam(x, y);

            User32.SendMessage(ptr, (uint)WindowsMessages.WM_LBUTTONDOWN, 0, lParam);
            return User32.SendMessage(ptr, (uint)WindowsMessages.WM_LBUTTONUP, 0, lParam);

        }

        public static int SendMouseRightClick(IntPtr ptr, int x, int y)
        {
            uint lParam = MakeLParam(x, y);

            User32.SendMessage(ptr, (uint)WindowsMessages.WM_RBUTTONDOWN, 0, lParam);
            return User32.SendMessage(ptr, (uint)WindowsMessages.WM_RBUTTONUP, 0, lParam);
        }

        /// <summary> 
        /// MakeLParam Macro 
        /// </summary> 
        public static uint MakeLParam(int x, int y)
        {
            //return ((y << 0x10) | x);
            return ((Convert.ToUInt32(y) << 16) | (Convert.ToUInt32(x) & 0xffff));
        }
        ...
        {
            IntPtr boxHwnd = GetDlgItem(hWnd, 0);
